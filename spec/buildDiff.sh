...
then
    ...
    cat $WORKDIR/spec/builds.txt | parallel --will-cite --ungroup --pipe -N50 "cat > /tmp/parallel_links_{#};  BUILDLINKS=/tmp/parallel_links_{#} BUILDCACHEPREFIX=${CACHEDIR} busted --lua=luajit -r generate" && \
...
then
    rm -rf /tmp/*
    cat $WORKDIR/spec/builds.txt | parallel --will-cite --ungroup --pipe -N50 'cat > /tmp/parallel_links_{#};  BUILDLINKS="/tmp/parallel_links_{#}" BUILDCACHEPREFIX="/tmp" busted --lua=luajit -r generate' && \
